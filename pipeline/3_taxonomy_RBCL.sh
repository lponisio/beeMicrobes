...
docker run -it \
  -v ~/University\ of\ Oregon\ Dropbox/Nevin\ Cullen/beeMicrobes_saved/beeMicrobes_pipeline_output:/mnt/beeMicrobes_pipeline_output \
...
## using R script called "3._taxonomy-refinement_RBCL.R"
...
qiime tools import \
  ...
  --input-path taxonomy_NCBI_RBCL.txt \
  --output-path taxonomyRBCL.qza

qiime metadata tabulate \
  --m-input-file taxonomyRBCL.qza \
  --o-visualization taxonomyRBCL.qzv
...
# can probably skip since we're just using NCBI
qiime feature-table filter-features \
  --i-table tableRBCL.qza \
  --m-metadata-file featurestokeep.txt \
...
qiime feature-table filter-seqs \
  --i-data rep-seqsRBCL.qza \
  --i-table tablefilt1.qza \
  --o-filtered-data rep-seqsRBCL-filtered.qza

qiime feature-table tabulate-seqs \
  --i-data rep-seqs-RBCL-filtered.qza \
  --o-visualization rep-seqsRBCL-filtered.qzv

qiime tools view rep-seqsRBCL-filtered.qzv
...
qiime phylogeny align-to-tree-mafft-fasttree \
  --i-sequences rep-seqsRBCL-filtered.qza \
...
qiime taxa barplot \
  --i-table split/tableR0.qza \
  --i-taxonomy taxonomyRBCL.qza \
  --m-metadata-file maps/SI2018mapRBCL.txt \
  --o-visualization split/taxa-bar-plotsR0.qzv
...
## use this list to write out code below where we remove specific sequences
...
#   --i-taxonomy taxonomyRBCL.qza \
#   --p-mode exact \
#   --p-exclude "Fagaceae sp. A; Apiaceae sp. C; Araceae sp. A" \
#   --o-filtered-table split/tableR1_f1.qza
...
qiime feature-table filter-samples \
  ...
  --o-filtered-table path/tableX_f2.qza
...
qiime taxa barplot \
  --i-table split/tableR0_f2.qza \
  --i-taxonomy taxonomyRBCL.qza \
  --m-metadata-file maps/sky2020mapRBCL_combined_repsremoved.txt \
  --o-visualization split/taxa-bar-plotsR0_f2.qzv
...
qiime feature-table summarize \
  ...
  --o-visualization split/tableR0_f2.qzv
...
qiime diversity core-metrics-phylogenetic \
  --i-phylogeny rooted-treeRBCL.qza \
  --i-table split/tableR0_f2.qza \
  --p-sampling-depth 1352 \
  --m-metadata-file maps/sky2020mapRBCL_combined_repsremoved.txt \
  --output-dir final/core_metricsRBCLR0 \
  --verbose

# pollen data is now prepared to combine with the rest of the specimen metadata in script: 4_specimenPrep.R